...
#pragma endregion
	...
	if (generate_new_pokemon)
	{
		...
		{
			cout << "Endpoint[" << _PlayerID << "]: 已产生新的系统精灵"<< endl;
		}
	}
...
void Endpoint::listenFunc()
{
	while (running)
	{
		...
		cout << "Endpoint[" << _PlayerID << "]成功与" << inet_ntoa(clientAddr.sin_addr) << "的用户连接" << endl;
...
void Endpoint::upgradeBattle(int pokemonid_1, int pokemonid_2)
{
	...
	cout << "Endpoint[" << _PlayerID << "]: 胜率已更新：win_round/total_round："<<win_round<<"/"<<total_round << endl;
}
